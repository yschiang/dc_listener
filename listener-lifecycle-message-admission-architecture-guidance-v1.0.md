...
8. Runtime reports ACTIVE or an explicit failure phase
```
...
| **L4** | Session-level desired state、observed phase 與 conditions | P1 |
...
Configuration readiness、connection、consumer 與 admission 的實際狀況，以 conditions 呈現，而不是各自維護獨立 lifecycle，也不為其建立額外 phase。

> **這是 Lifecycle Model 的設計約束，不是獨立的 Architecture Principle。**

為避免 Guidance 膨脹成 implementation state machine，本章只保留對操作人員與系統行為具有明確語意的 phase。

例如，實作內部可能需要 `STOPPING`、`RECONNECTING` 或 `DRAIN_TIMEOUT_PENDING` 等 transient state，但不必全部成為 Architecture Guidance 的公開 phase。
...
Desired state 與 successful observed phase 的主要映射：

| Desired State | Successful Observed Phase |
|---|---|
| `RUNNING` | `ACTIVE` |
| `STANDBY` | `STANDBY` |
| `STOPPED` | `STOPPED` |

`STANDBY` 與 `STOPPED` 在 desired state 與 observed phase 使用相同名稱，但分別代表：
...
Architecture Guidance 保留七個 observed phases：
...
這些 phase 是 Runtime observed state，不應由操作人員直接任意指定。
...
但不需要將它暴露為本文件的核心 architecture phase。

---

## 8.4 Conditions

Lifecycle phase 之外，以 conditions 呈現實際 operational status：
...
Conditions 用於呈現 phase 內部的重要 observed facts，而不是建立另一套 lifecycle。
...
Lifecycle phase 與 actions 的預設關係：

| Phase | Connection Action | Consumption | Admission |
|---|---|---|---|
...
- Listener Runtime 如何回報 observed phase 與 conditions？
...
- Lifecycle phase 與 conditions 如何更新？
...
Architecture Guidance 只保留七個具備明確操作語意的 observed phases：
...
實作內部所需的其他 transient state，應由 Technical Design 定義，而不需要全部暴露為 Architecture Guidance 的公開 phase。

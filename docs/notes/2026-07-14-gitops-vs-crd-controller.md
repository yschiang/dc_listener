...
以「每次變更都要 rollout」的最原本為基線，三步進化各動**不同的軸**，收益疊加：

```text
最原本：config 位於 Pod template/image ─── 任何變更 = rollout + 變更單
   │
   │ (1) runtime 軸【幅度：大——整個 listener runtime；已完成 = 本 repo refactor】
   ▼
A: 單 session Pod + watch 共用大檔（refactor 現狀）
   ├─ 得到：dynamic 變更零 rollout、process/Pod 隔離、DEGRADED/恢復語意、
   │        last-good 護底、durable 所有權安全（T2）
   ├─ 尚痛：一大包（全體漣漪、全域爆炸半徑）、status 斷頭、退場 runbook、
   │        變更照走變更單
   │
   │ (2) 交付軸【幅度：小——render 切檔 + Pod volume 指向 per-tool CM；
   │     runtime 零變更（SpecParser 忽略未知根鍵，metadata 塊可先行搭載）】
   ▼
B: per-tool projection（中繼版 = 實戰版）
   ├─ 得到：爆炸半徑縮到單 tool、漣漪 1 個 Pod、per-tool metadata 有家、
   │        gate 8 測試提前立起
   ├─ 尚痛：status 斷頭、退場 runbook、變更單照走
   │
   │ (3) 真相/控制軸【幅度：大——CRD + controller + CEL admission + kind 環境 + 遷移】
   ▼
C: ToolListener CRD + controller（demo 先行，觸發條件到才 production 化）
   └─ 得到：status 閉環、退場自動化（finalizer）、寫入時擋錯、
            （假設證實後）dynamic 變更脫離變更單流程
```

**A 與 B 的關係——時間上可並行，語意上是疊加不是二選一：**

- 動的層不同：A 是 runtime 行為（本 repo，T6/T7 收尾中）；B 是交付形狀
  （GitOps repo/deploy 層，runtime 零變更）。檔案不相交，**可同時進行**。
- 但 B 不是繞過 A 的另一條路：B 依賴 A 建立的「watch 掛載檔、熱 reconcile、
  last-good」契約——沒有 A，per-tool ConfigMap 無物可投。
- C 再疊在 A+B 上：controller 只是接管「誰來寫這些 per-tool CM」，B 完成後
  交付形狀已是 C 要的，C 的手術範圍因此縮小。

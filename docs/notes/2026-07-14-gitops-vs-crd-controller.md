...
```text
最原本：config 內嵌在 deployment 裡 ─── 改任何東西 = rollout + 變更單
   │
   │ (1) 把「執行」與「config」分開【幅度：大；已完成 = 本 repo refactor】
   ▼
A: 每個 tool 一個獨立 Pod；config 外部化、運行中 hot reload（現狀）
   ├─ 得到：改 config 不用 rollout、tool 之間故障互不牽連、斷線降級自癒、
   │        壞 config 不倒站（沿用 last-good）、durable 身分不會被誤刪（T2）
   ├─ 尚痛：所有 tool 共用一份 config——牽一髮動全身；看不到 applied 了沒；
   │        下架靠人工；變更單照走
   │
   │ (2) config 按 tool 分裝【幅度：小——只動打包/掛載方式，runtime 零變更】
   ▼
B: 每個 tool 一份 config（中繼版 = 實戰版）
   ├─ 得到：改誰只動誰、寫壞一份只影響一個 tool、每份 config 可蓋上
   │        身分/版本戳記、「改 config 不觸發 rollout」從此有測試把關（gate 8）
...
C: ToolListener CRD + controller（demo 先行，觸發條件到才 production 化）
   └─ 得到：declared 與 applied 同處對帳（看得到 applied 了沒）、上下架
            自動化且有安全握手、寫錯當場被擋、（假設證實後）改 config 免變更單
```

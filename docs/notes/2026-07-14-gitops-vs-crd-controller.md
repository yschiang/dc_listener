...
```text
最原本：組態內嵌在部署描述裡 ─── 調任何東西 = 重新部署 + 變更單
   │
   │ (1) 把「執行」與「組態」分開【幅度：大；已完成 = 本 repo refactor】
   ▼
A: 一個工具一個獨立執行單元；組態外部化、運行中熱套用（現狀）
   ├─ 得到：調組態不用重佈、工具之間故障互不牽連、斷線會降級自癒、
   │        壞組態不會弄倒服務（沿用最後一份好的）、消費身分不會被誤刪（T2）
   ├─ 尚痛：全部工具仍共用一份組態——牽一髮動全身；看不到「套用了沒」；
   │        下架靠人工；變更單照走
   │
   │ (2) 組態按工具分裝【幅度：小——只動打包與掛載方式，程式零變更】
   ▼
B: 一個工具一份組態（中繼版 = 實戰版）
   ├─ 得到：改誰只動誰、一份寫壞只影響一個工具、每份組態可蓋上
   │        身分與版本戳記、「改組態不會觸發重佈」從此有測試把關（gate 8）
   ├─ 尚痛：看不到「套用了沒」；下架靠人工；變更單照走
   │
   │ (3) 期望狀態升級為正式物件，交給控制器對帳【幅度：大——CRD + controller
   │     + 寫入把關 + kind 測試環境 + 身分遷移】
   ▼
C: ToolListener CRD + controller（demo 先行，觸發條件到才 production 化）
   └─ 得到：宣告與實況同處對帳（看得到「套用了沒」）、上下架自動化且有
            安全握手、寫錯在寫入當下就被擋、（假設證實後）調整免變更單
```

**A 與 B 的關係——時間上可並行，語意上是疊加不是二選一：**

- 解的問題不同：A 管「程式怎麼消化組態」（本 repo，T6/T7 收尾中）；B 管
  「組態怎麼送到程式面前」（GitOps repo/deploy 層，程式零變更）。互不碰對方
  的檔案，**可同時進行**。
- 但 B 不是繞過 A 的另一條路：「改組態即時生效、壞檔不倒站」是 A 建立的能力
  ——沒有 A，B 分裝好的組態送到了也沒人會熱套用。
- C 再疊在 A+B 上：控制器只是接管「誰來寫每個工具那份組態」，B 完成後組態的
  形狀已經是 C 要的，C 的手術範圍因此縮小。
